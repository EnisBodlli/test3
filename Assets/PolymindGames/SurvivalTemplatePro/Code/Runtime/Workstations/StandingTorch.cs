...
namespace PolymindGames.BuildingSystem
{
    ...
    public sealed class StandingTorch : Interactable, ISaveableComponent
    {
        ...
        [SyncVar(hook = nameof(OnFireEnabledCallBack))] 
        ...
        private void OnFireEnabledCallBack(bool old, bool newValue)
        {
            if (m_FireEnabled)
            {
            ...
            Description = m_FireEnabled ? m_DisableFireText : m_EnableFireText;

            if (m_VolumeSetter != null)
                StopCoroutine(m_VolumeSetter);

            m_VolumeSetter = StartCoroutine(C_SetAudioVolume(m_FireEnabled ? m_FireAudioVolume : 0f));
        }
