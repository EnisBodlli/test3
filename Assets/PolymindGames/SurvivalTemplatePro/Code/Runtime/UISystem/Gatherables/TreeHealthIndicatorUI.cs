...
namespace PolymindGames.UISystem
{
    public class TreeHealthIndicatorUI : PlayerUIBehaviour
    {
        ...
        public static void ShowIndicator(GatherableDefinition[] definitions)
        {
            ...
            if (s_Instance.m_AllTreesInScene == null)
                s_Instance.m_AllTreesInScene = new();
            else
                s_Instance.m_AllTreesInScene.Clear();

            foreach (var def in definitions)
            {
                Gatherable.TryGetAllGatherablesWithDefinition(def, out var list);
                s_Instance.m_AllTreesInScene.AddRange(list);
            }
        ...
        private void UpdateIndicator()
        {
            ...
            if (m_AllTreesInScene.Count > 0)
            {
                ...
                foreach (var tree in m_AllTreesInScene)
                {
                    ...
                    // Adjust angle calculation to make the tree easier to hit
                    ...
                    // Allow the interaction above and below by modifying the angle limit
                    if (m_DistanceFromTree < m_MaxShowDistance && m_AngleFromTree < (m_ShowAngle + 30f) && tree.Health > Mathf.Epsilon)
                    {
                        m_ClosestTree = tree;
