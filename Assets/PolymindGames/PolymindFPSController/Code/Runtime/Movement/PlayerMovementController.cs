...
namespace PolymindGames.MovementSystem
{
    public sealed class PlayerMovementController : CharacterBehaviour, IMovementController, ISaveableComponent
    {
        ...
        {
            //if (Character.isLocalPlayer == false) return;

            //Debug.Log("OBJECT ON Enabled" + Character.gameObject.name);
        ...
        public void SetMovementPreset(MovementPreset preset)
        {
            ...
            // Don't initialize it on proxies. as this is based on scriptable objects
            if (Character.isLocalPlayer == false) return;
        ...
        #region Update Loop
        public Vector3 GetMotionInput(Vector3 velocity, out bool useGravity, out bool snapToGround)
        {
