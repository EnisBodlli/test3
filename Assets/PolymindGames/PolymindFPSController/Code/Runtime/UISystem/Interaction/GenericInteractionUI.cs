...
namespace PolymindGames.UISystem
{
    ...
    public sealed class GenericInteractionUI : MonoBehaviour, IInteractableInfoDisplayer
    {
        ...
        public void UpdateInfo(IHoverable hoverable)
        {
            ...
            IWorkstation workstation = hoverable.gameObject.GetComponent<IWorkstation>();

            if (workstation != null && workstation.InUse)
            {
                string inuse = workstation.InUse == true ? " <color=red>(In Use)</color>" : "";
                m_DescriptionText.text = m_AttachedHoverable.Description + inuse;
                return;
            }
