...
public class CentralizedMonsterSpawner : NetworkBehaviour
{
    ...
    private void DamageTarget(GameObject target)
    {
        ...
        // Find the HealthManager in the target or its children
        var healthManager = target.GetComponentInChildren<HealthManager>();
        if (healthManager != null)
        {
            healthManager.ReceiveDamageCmd(damage);  // Apply damage using the existing ReceiveDamageCmd method
        }
        else
        {
            Debug.LogError("Target or its children do not have a HealthManager component.");
        }
    }
